...
make build|test|release|install|run|clean|help   # Makefile wrappers
```
...
Executable target `BatchOCR` (product `batch-ocr`); tests `@testable import BatchOCR`. Flow: `BatchOCRCommand` (ArgumentParser, maps flags → `OCRConfig`) → `FileDiscovery` → `BatchProcessor` (sliding-window TaskGroup capped at `--jobs`) → `Reporter`. OCR lives only in `VisionOCREngine` behind the `OCREngine: Sendable` protocol — tests inject `MockEngine`. Exit codes are a contract: 0 ok, 1 per-file failures, 2 usage/config errors.
...
- `Reporter` output is machine-readable: keep `en_US_POSIX` locale for `elapsed=` and log timestamps.
...
- `--language` defaults to `[]` with fallback to `["en"]` in `makeConfig()`; don't give the option a non-empty default (ArgumentParser array-default semantics).
